"""
convert_idl.py
Converts an Anchor 0.32 IDL (crowdfunding.json) to the old format expected
by anchorpy-core 0.2.0 / solders 0.26.0.

Key transformations:
  - Top-level: extract 'name'+'version' from 'metadata'; drop 'address' at root
  - Instruction accounts: writable->isMut, signer->isSigner; drop address/relations
  - PDA seeds: add missing 'type' field (const->bytes, account->publicKey, arg->u64)
  - PDA program override: rename 'program'->'programId', add type:'publicKey'
  - Types: 'pubkey' -> 'publicKey'
  - accounts[]: merge Anchor 0.32 discriminator list with struct defs from types[]
"""
