...
Benchmark script: `benchmarks/run_tests.py --platform solana` with `VARIANT=V5`.
...
> Run `VARIANT=V5 python benchmarks/run_tests.py --platform solana` with a running `solana-test-validator`
> to populate this section. Result: `benchmarks/results/V5_python_solana-localnet_lifecycle_{TIMESTAMP}.json`.
...
> EVM latency source: `benchmarks/run_tests.py --platform evm` (2026-03-15, Hardhat in-process EVM).
> Solana latency source: M-V4-1 (2026-03-07, `solana-test-validator` via `benchmarks/run_tests.py --platform solana`).
...
> EVM source: `benchmarks/throughput_test.py --platform evm` (2026-03-15). Solana source: M-V4-1
> via `benchmarks/run_tests.py --platform solana` (2026-03-07). Both benchmarks pre-create
> accounts / approvals outside the timed window and submit contributions sequentially,
> waiting for confirmation before each next call.
