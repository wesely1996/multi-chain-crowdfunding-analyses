"""
evm/withdraw.py -- Withdraw a milestone from a successful campaign.
...
def withdraw(variant: str) -> TxOutput:
    """Call withdrawMilestone() on the campaign contract."""
    w3 = get_web3()
    ...
    receipt, latency = build_and_send(w3, campaign.functions.withdrawMilestone(), account, gas=200_000)
...
def main(args=None) -> None:
    parser = argparse.ArgumentParser(description="Withdraw a milestone.")
    parser.add_argument("--variant", default=evm_config.VARIANT)
    parsed = parser.parse_args(args)

    try:
        output = withdraw(parsed.variant)
        print_result(output)
    except Exception as exc:
        print_error("withdraw", exc, chain="evm")
