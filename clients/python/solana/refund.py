"""
solana/refund.py -- Refund a contributor from a failed Solana campaign.
...
async def _refund(variant: str) -> TxOutput:
    """Call refund on the campaign."""
...
def main(args=None) -> None:
    parser = argparse.ArgumentParser(description="Refund a contributor from a Solana campaign.")
    parser.add_argument("--variant", default=sol_config.SOLANA_VARIANT)
    parsed = parser.parse_args(args)

    try:
        output = asyncio.run(_refund(parsed.variant))
        print_result(output)
    except Exception as exc:
        print_error("refund", exc, chain="solana")
